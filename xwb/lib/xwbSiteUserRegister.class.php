...
class xwbSiteUserRegister{
	...
	function reg( $name, $email, $pwd= false ){
		$this->username = DB::mysqli_escape(trim($name));
		$this->email = DB::mysqli_escape(trim($email));
		$this->password = $pwd ? DB::mysqli_escape($pwd) : rand(100000,999999);
	...
	function _updateRegCTR(){
		...
		if( $this->regfloodcount == 0 ){
			...
			DB::query("UPDATE ". DB::table('common_regip'). " SET count=count+1 WHERE ip='{$this->ip}' AND count>'0'");
		}
		if ($_G ['setting'] ['regctrl'] || $_G ['setting'] ['regfloodctrl']) {
			DB::query( "DELETE FROM ". DB::table('common_regip'). " WHERE dateline<='{$this->timestamp}'-" . ($_G ['setting'] ['regctrl'] > 72 ? $_G ['setting'] ['regctrl'] : 72) . "*3600", 'UNBUFFERED' );
	...
	function _updateUserRegVerify(){
		...
		//EMAIL验证
			...
			DB::query("UPDATE ". DB::table('common_member_field_forum'). " SET authstr='$authstr' WHERE uid='{$this->uid}'");
	...
	function _checkRegIP(){
		...
		if ($_G ['setting'] ['regctrl']) {
			$result = DB::result_first( "SELECT ip FROM " . DB::table('common_regip'). " WHERE ip LIKE '$ctrlip' AND count='-1' AND dateline>{$this->timestamp}-'" . $_G ['setting'] ['regctrl'] . "'*3600 LIMIT 1" );
			if (!empty($result)) {
				return -1002;
			}
		}
		...
		if ($_G ['setting'] ['regfloodctrl']) {
			if ($regattempts = DB::result_first( "SELECT count FROM " . DB::table('common_regip'). " WHERE ip='{$this->ip}' AND count>'0' AND dateline>'{{$this->timestamp}}'-86400" )) {
