...
class siteBindMapper{
	...
	function midMapper2all($mids){
		...
		$query = DB::query('SELECT * FROM '. DB::table('xwb_bind_thread'). " WHERE `mid` IN (". implode(',', $mids). ")");
	...
	function tidMapper($tids){
		...
		$fidMap = array();
		$query = DB::query('SELECT * FROM '. DB::table('forum_thread'). " WHERE `tid` IN (". implode(',', $tids). ')');
		while( $row = DB::fetch($query) ){
	...
	function fidMapper( $fids ){
		...
		$query = DB::query('SELECT fid, fup, type FROM '. DB::table('forum_forum'). " WHERE `fid` IN (". implode(',', $fids). ')');
		while( $row = DB::fetch($query) ){
	...
	function blogMapper($blogids){
		...
		$query = DB::query('SELECT blogid,uid FROM '. DB::table('home_blog'). " WHERE `blogid` IN (". implode(',', $blogids). ')');
		while( $row = DB::fetch($query) ){
	...
	function shareMapper($sids){
		...
		$query = DB::query('SELECT sid, uid FROM '. DB::table('home_share'). " WHERE `sid` IN (". implode(',', $sids). ')');
		while( $row = DB::fetch($query) ){
	...
	function doingMapper($doids){
		...
		$query = DB::query('SELECT doid, uid FROM '. DB::table('home_doing'). " WHERE `doid` IN (". implode(',', $doids). ')');
		while( $row = DB::fetch($query) ){
