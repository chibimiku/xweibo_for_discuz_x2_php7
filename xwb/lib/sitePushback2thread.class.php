...
class sitePushback2thread{
	...
	function _prepareSqlData( $tid, $content, $time ){
		...
		$lastpost = DB::mysqli_escape("{$tid}\t{$tidInfo['subject']}\t{$time}\t{$username}" );
	...
	function execInsert(){
		...
		if ( false == $this->_checkPostPeriods() ){
			foreach ($this->_threadData as $tid => $tidInfo){
				DB::query("UPDATE ". DB::table('forum_thread'). " SET lastposter='{$tidInfo['lastposter']}', lastpost='{$tidInfo['lastpost']}', replies=replies+{$tidInfo['replies']} WHERE tid='{$tid}'", 'UNBUFFERED');
			}
		}
		...
		if( $this->_userConfig['uid'] > 0 && $this->_userReplyCount > 0  ){
			DB::query("UPDATE ". DB::table('common_member_count'). " SET posts=posts+{$this->_userReplyCount} WHERE uid='{$this->_userConfig['uid']}'", 'UNBUFFERED');
		}
		...
		foreach ( $this->_forumData as $fid => $fidInfo ){
			if( false == $this->_checkPostPeriods() ){
				DB::query("UPDATE ". DB::table('forum_forum'). " SET lastpost='{$fidInfo['lastpost']}', posts=posts+{$fidInfo['posts']}, todayposts=todayposts+{$fidInfo['posts']} WHERE fid='$fid'", 'UNBUFFERED');
			}else{
				DB::query("UPDATE ". DB::table('forum_forum'). " SET todayposts=todayposts+{$fidInfo['todayposts']} ,modworks='1' WHERE fid='$fid'", 'UNBUFFERED');
			}
