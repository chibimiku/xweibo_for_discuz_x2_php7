...
class session_storage_db{
	 ...
	 */
	function read($id){
		$id = DB::mysqli_escape( $id);
		$result = DB::result_first( 'SELECT `data` FROM '. DB::table($this->_sessionTable). 
												" WHERE `sessionid` = '{$id}' " 
											);
	...
	function write($id, $sess_data){
		$id = DB::mysqli_escape( $id);
		$sess_data = DB::mysqli_escape( $sess_data);
		$lasttime = time();
		
		DB::query('REPLACE INTO '. DB::table($this->_sessionTable). 
	...
	function destroy($id){
		$id = DB::mysqli_escape( $id);
		DB::query('DELETE FROM '. DB::table($this->_sessionTable). 
							" WHERE `sessionid` = '{$id}' "
							);
	}
